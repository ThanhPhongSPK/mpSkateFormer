...
def create_mediapipe_skateformer(num_classes=60):
    # Temporal: T=64. Partition theo thời gian (Local & Global)
    # Skeletal: V=36. Partition theo không gian (Neighboring & Distant)
    
    # Type 1: Neighboring Joints (L=4) + Local Motion (N=8 frames)
    # => type_1_size = (8, 4)
    
    # Type 2: Distant Joints (K=9) + Local Motion (N=8 frames)
    # => type_2_size = (8, 9)  <-- Distant size K=V/L = 36/4 = 9
    
    # Type 3: Neighboring Joints (L=4) + Global Motion (M=8 segments)
    # => type_3_size = (8, 4)
    
    # Type 4: Distant Joints (K=9) + Global Motion (M=8 segments)
    # => type_4_size = (8, 9)
